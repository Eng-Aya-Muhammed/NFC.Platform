  ...
  <table width=""100%"" cellpadding=""0"" cellspacing=""0"" style=""background:#f4f6f9;padding:40px 0;"">
    <tr><td align=""center"">
      <table width=""560"" cellpadding=""0"" cellspacing=""0"" style=""background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);"">
        ...
        public async Task SendTemplateRequestApprovedEmailAsync(string to, string templateName, string culture)
        {
            SetThreadCulture(culture);

            var subject = _messageService.Get("EmailTemplateApprovedSubject");
            var subtitle = _messageService.Get("EmailTemplateApprovedSubtitle");
            var greeting = _messageService.Get("EmailTemplateApprovedGreeting");
            var bodyPattern = _messageService.Get("EmailTemplateApprovedBody");
            var footerText = _messageService.Get("EmailTemplateApprovedFooter");

            var bodyText = string.Format(bodyPattern, templateName);
